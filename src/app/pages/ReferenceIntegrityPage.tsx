...
// and assess whether it supports the in-text claim attributed to it. Every row
// is a calibrated, evidence-linked flag for human triage — reviewer-assist.
...
import {
  BookMarked, Play, Loader2, X, Check, AlertTriangle, ExternalLink, Ban, FileQuestion,
} from "lucide-react";
import {
  ReferenceIntegrityService, type RefInput, type RefResult, type RefSummary, type RefSeverity,
} from "../lib/apiClient";

const EXAMPLE = `10.1016/S0140-6736(97)11096-0 | MMR vaccination causes autism in children.
10.1038/s41586-020-2649-2 | NumPy enables array programming and was shown to cure diabetes.
10.1038/s41586-020-2649-2 | NumPy provides fast array programming for scientific computing in Python.
10.9999/this-doi-does-not-exist-123 | A landmark trial proved the intervention doubled survival.
Attention is all you need, Vaswani et al., NeurIPS 2017 | The Transformer architecture relies on self-attention instead of recurrence.`;
...
const VERDICT_LABEL: Record<string, string> = {
  supports: "Supports", partial: "Partial", unsupported: "Unsupported",
  unrelated: "Unrelated", unverifiable: "Unverifiable", no_claim: "No claim",
...
// Parse the textarea: one reference per line, "<doi or citation> | <claim>".
  ...
  async function run() {
    ...
    try {
      const { summary } = await ReferenceIntegrityService.check(refs, {
        ...
        signal: ac.signal,
        // Append each flag as it streams in, keeping the list index-sorted.
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              (optionally) verify the reference actually supports the in-text claim. Each row is a flag
              for you to accept or dismiss — reviewer-assist, never an automated accusation.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-5 space-y-3">
        <div className="flex items-center justify-between">
          <Label>References — one per line, optionally <code className="text-[11px] bg-muted px-1 rounded">DOI or citation | in-text claim</code></Label>
          <Button variant="ghost" size="sm" onClick={() => setInput(EXAMPLE)} disabled={running}>Load example</Button>
        </div>
        <Textarea
          value={input}
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          {running ? (
            <Button variant="outline" onClick={cancel}><X className="size-4 mr-1.5" />Cancel</Button>
          ) : (
        ...
        {checkClaims && (
          <p className="text-[11px] text-muted-foreground">
            Claim support needs a working model. Pick one in the sidebar — set a cloud key in <code className="bg-muted px-1 rounded">Backend/.env</code> or run a local Ollama model. Resolution + retraction work without a model.
          </p>
        )}
      </Card>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {r.resolved ? (
              <a href={r.matched.url || "#"} target="_blank" rel="noreferrer"
            ...
            {r.matched.providers?.length ? (
              <span className="text-[10px] text-muted-foreground">via {r.matched.providers.join(" + ")}</span>
            ) : null}
          </div>

          {/* Cited-as line */}
          <div className="text-xs text-muted-foreground truncate">
            Cited as: <span className="font-mono">{cited}</span>
          </div>
          ...
          {claimV && claimV !== "skipped" && claimV !== "no_claim" && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge variant="outline" className={VERDICT_STYLE[claimV] || "bg-muted"}>
                {VERDICT_LABEL[claimV] || claimV}
              </Badge>
              ...
              {(r.claim.reasoning || r.claim.quote) && (
                <Popover>
                  <PopoverTrigger asChild>
                    <button className="text-[11px] text-primary hover:underline">why?</button>
                  </PopoverTrigger>
        ...
        <div className="flex flex-col gap-1.5 shrink-0">
          <Button variant={decision === "accept" ? "default" : "outline"} size="sm" className="h-7 px-2"
            onClick={() => onDecide("accept")} title="Accept this flag">
            <Check className="size-3.5" />
          </Button>
          <Button variant={decision === "dismiss" ? "secondary" : "outline"} size="sm" className="h-7 px-2"
            onClick={() => onDecide("dismiss")} title="Dismiss this flag">
