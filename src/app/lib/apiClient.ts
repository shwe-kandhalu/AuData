...
export const ReferenceIntegrityService = {
  // Streams one result per reference as it finishes, then resolves with all
  // results + the summary. Mirrors the backend SSE format.
  async check(
    references: RefInput[],
    opts: {
      checkClaims?: boolean;
    ...
    const res = await fetch(`${apiConfig.baseUrl}/reference-integrity/check/stream`, {
      ...
      body: JSON.stringify({
        references,
        model: apiConfig.model,
        check_claims: opts.checkClaims !== false,
        task_id: opts.taskId,
      }),
      signal: opts.signal,
    });
    ...
    while (true) {
      ...
      while ((idx = buf.indexOf("\n\n")) !== -1) {
        ...
        try { parsed = JSON.parse(data); } catch { continue; }
        if (event === "result") {
          results.push(parsed as RefResult);
          opts.onResult?.(parsed as RefResult);
        } else if (event === "done") {
          ...
          throw new Error(parsed?.message || "reference-integrity error");
        }
      }
    }
