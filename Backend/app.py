...
def extract_tables_from_paper(paper_title: str, paper_url: str, paper_source: str, paper_id: str, extraction_type: str) -> list:
    ...
    try:
        ...
        # Method 3: Try Unpaywall to find open access PDF
        if not tables and paper_id:
            print(f"Attempting Unpaywall lookup for PMID: {paper_id}")
            unpaywall_tables = fetch_unpaywall_tables(paper_id, paper_title)
            if unpaywall_tables:
                tables.extend(unpaywall_tables)
                print(f"Added {len(unpaywall_tables)} tables from Unpaywall PDF")
        
        # Method 4: Try PMC direct article fetch (different endpoint)
        if not tables and paper_id:
            print(f"Attempting PMC direct fetch for PMID: {paper_id}")
            pmc_direct_tables = fetch_pmc_direct_tables(paper_id)
            if pmc_direct_tables:
                tables.extend(pmc_direct_tables)
                print(f"Added {len(pmc_direct_tables)} tables from PMC direct")
...
def fetch_epmc_tables(paper_id: str) -> list:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  [EPMC] ERROR: HTTP {response.status_code} - Paper not available in Europe PMC (may be paywalled)")
...
def fetch_pmc_tables(paper_id: str) -> list:
    ...
    try:
        ...
        try:
            ...
            # Handle the resolver error - paper not in PMC
            if "address table is empty" in str(e) or "couldn't resolve" in str(e).lower():
                print(f"  [PMC] Paper {paper_id} is not available in PubMed Central (not open access)")
                return tables
            else:
        ...
        return tables  # Return empty tables on error, don't crash
    
    print(f"  [PMC] Returning {len(tables)} tables")
    return tables


def fetch_unpaywall_tables(paper_id: str, paper_title: str) -> list:
    """
    Use Unpaywall API to find open access PDF and extract tables.
    Unpaywall finds legally available open access versions of paywalled papers.
    """
    tables = []
    
    print(f"  [Unpaywall] Starting lookup for PMID: {paper_id}")
    
    if not paper_id:
        return tables
    
    try:
        # Step 1: Get DOI from PMID using PubMed
        from Bio import Entrez
        from config import Config
        
        Entrez.email = Config.ENTREZ_EMAIL
        
        print(f"  [Unpaywall] Looking up DOI for PMID {paper_id}...")
        ...
        if not doi_elem:
            print(f"  [Unpaywall] No DOI found for PMID {paper_id}")
            return tables
        
        doi = doi_elem.get_text(strip=True)
        print(f"  [Unpaywall] Found DOI: {doi}")
        
        # Step 2: Query Unpaywall API
        unpaywall_url = f"https://api.unpaywall.org/v2/{doi}?email={Config.ENTREZ_EMAIL}"
        print(f"  [Unpaywall] Calling API: {unpaywall_url[:60]}...")
        ...
        if data.get('best_oa_location'):
            ...
            print(f"  [Unpaywall] Best OA PDF URL: {pdf_url}")
        
        # If no PDF from best, try all OA locations
        if not pdf_url and data.get('oa_locations'):
            for loc in data['oa_locations']:
                if loc.get('url_for_pdf'):
                    pdf_url = loc['url_for_pdf']
                    print(f"  [Unpaywall] Found PDF in OA locations: {pdf_url}")
                    break
        
        if not pdf_url:
            print(f"  [Unpaywall] No open access PDF available")
            is_oa = data.get('is_oa', False)
            print(f"  [Unpaywall] Paper is OA: {is_oa}")
            return tables
        
        # Step 3: Try to fetch and parse the PDF
        print(f"  [Unpaywall] Attempting to fetch PDF...")
        tables = extract_tables_from_pdf(pdf_url, paper_title)
        
    except Exception as e:
        print(f"  [Unpaywall] EXCEPTION: {e}")
    
    print(f"  [Unpaywall] Returning {len(tables)} tables")
    return tables


def extract_tables_from_pdf(pdf_url: str, paper_title: str) -> list:
    """
    Download PDF and extract tables using tabula-py or pdfplumber.
    """
    tables = []
    
    print(f"  [PDF] Starting extraction from: {pdf_url[:60]}...")
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"  [PDF] Failed to download: HTTP {response.status_code}")
            return tables
        
        pdf_bytes = response.content
        print(f"  [PDF] Downloaded {len(pdf_bytes)} bytes")
        
        # Try pdfplumber (pure Python, easier to install)
        try:
            import pdfplumber
            ...
            with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if page_tables:
                        print(f"  [PDF] Page {page_num}: Found {len(page_tables)} tables")
                        
                        for i, table in enumerate(page_tables):
                            if table and len(table) >= 1:
                                tables.append({
                                    'title': f"Table {len(tables)+1} (PDF Page {page_num})",
                                    'type': classify_table_type(table, 'All Tables'),
                                    'data': table
                                })
                                print(f"    -> Added table with {len(table)} rows")
            
            if tables:
                print(f"  [PDF] Successfully extracted {len(tables)} tables")
                return tables
                
        except ImportError:
            print(f"  [PDF] pdfplumber not installed, trying alternative...")
        
        # Fallback: Try camelot (more accurate but harder to install)
        try:
            import camelot
            
            print(f"  [PDF] Using camelot for extraction...")
            ...
            print(f"  [PDF] Camelot found {len(camelot_tables)} tables")
            ...
            for i, table in enumerate(camelot_tables):
                ...
                if len(df) >= 1:
                    ...
                    tables.append({
                        'title': f"Table {i+1} (PDF)",
                        'type': classify_table_type(table_data, 'All Tables'),
                        'data': table_data
                    })
                    print(f"    -> Added table with {len(table_data)} rows")
            
            # Clean up temp file
            os.unlink(tmp_path)
            
        except ImportError:
            print(f"  [PDF] camelot not installed either")
        except Exception as e:
            print(f"  [PDF] Camelot error: {e}")
        
        if not tables:
            print(f"  [PDF] No PDF extraction libraries available or no tables found")
            print(f"  [PDF] To enable PDF table extraction, install: pip install pdfplumber")
...
def fetch_pmc_direct_tables(paper_id: str) -> list:
    """
    Alternative method to fetch PMC article directly using different API endpoint.
    Sometimes papers are in PMC but elink doesn't find them.
    """
    tables = []
    
    print(f"  [PMC-Direct] Starting extraction for PMID: {paper_id}")
    ...
    try:
        ...
        # Method 1: Try to search PMC directly for this PMID
        print(f"  [PMC-Direct] Searching PMC database for PMID {paper_id}...")
        
        handle = Entrez.esearch(db="pmc", term=f"{paper_id}[pmid]", retmax=1)
        ...
        if not pmc_ids:
            print(f"  [PMC-Direct] No PMC article found for PMID {paper_id}")
            return tables
        
        pmc_id = pmc_ids[0]
        print(f"  [PMC-Direct] Found PMC ID via search: {pmc_id}")
        ...
        print(f"  [PMC-Direct] Got XML content: {len(xml_content)} bytes")
        
        # Parse XML to find tables
        soup = BeautifulSoup(xml_content, 'xml')
        
        # Find all table-wrap elements
        table_wraps = soup.find_all('table-wrap')
        print(f"  [PMC-Direct] Found {len(table_wraps)} <table-wrap> elements")
        ...
        for i, table_wrap in enumerate(table_wraps):
            ...
            # Extract rows from thead/tbody or all rows
            thead = table_wrap.find('thead')
            if thead:
            ...
            if not table_data:
                for row in table_wrap.find_all('tr'):
                    row_data = [cell.get_text(strip=True) for cell in row.find_all(['th', 'td'])]
                    if row_data:
                        table_data.append(row_data)
            ...
            if table_data and len(table_data) >= 1:
                ...
                print(f"    -> Added {label_text}: {len(table_data)} rows")
    
    except Exception as e:
        print(f"  [PMC-Direct] EXCEPTION: {e}")
    
    print(f"  [PMC-Direct] Returning {len(tables)} tables")
    return tables
...
def main():
    ...
    # --- PAGE 3: ABSTRACT SCREENING ---
        ...
        if st.session_state.results is not None and not st.session_state.results.empty:
            ...
            # Show agent scores summary with timing
            ...
            with col4:
                duration = st.session_state.get('screening_duration', 0)
                st.metric("Time", format_duration(duration))
            ...
            for idx, row in display_results.iterrows():
                ...
                for agent_name, vote_info in trace.items():
                    ...
                    agent_columns[agent_name].append(vote)
            ...
            def color_decision(val):
                ...
                elif 'EXCLUDE' in val_str or 'FAIL' in val_str:
        ...
        # Status container for searching and screening (appears above button)
        status_container = st.container()
        
        # SINGLE BUTTON at the bottom of the tab
        if st.session_state.history:
            st.write("---")
            if st.button("Run Database Search", type="primary", use_container_width=True, key="run_search_tab"):
                with status_container.status("🔍 Searching and AI-Screening...", expanded=True) as status:
                    # 1. Fetching
                    ...
                    # Track start time for screening
                    screening_start_time = time.time()
                    
                    # Progress tracking with more granular updates
                    progress_bar = st.progress(0)
                    status_text = st.empty()
                    sub_progress = st.empty()  # For sub-step progress
                    
                    def agent_progress(current, total, message=""):
                        """Track agent completion progress with granular updates."""
                        progress_pct = current / total
                        progress_bar.progress(progress_pct)
                        status_text.text(f"📄 Paper {current}/{total}: {message[:60]}...")
                        
                        # Show sub-progress based on worker batches
                        workers = Config.PARALLEL_SCREENING_WORKERS
                        batch_size = max(1, total // workers)
                        current_batch = (current - 1) // batch_size + 1
                        total_batches = (total + batch_size - 1) // batch_size
                        sub_progress.caption(f"Batch {current_batch}/{total_batches} | Workers: {workers} parallel")
                    
                    # Show initial search phase progress
                    status_text.text("🔍 Phase 1/3: Fetching papers from databases...")
                    sub_progress.caption(f"Sources: {', '.join(active_sources)}")
                    ...
                    # 6. Save Results with timing
                    elapsed_time = time.time() - screening_start_time
                    st.session_state.screening_duration = elapsed_time
                    st.session_state.results = pd.DataFrame(screened)
                    formatted_time = format_duration(elapsed_time)
                    status.update(label=f"✅ Found {len(screened)} papers! ({formatted_time})", state="complete")
                    st.rerun()
                ...
                if 'full_text_results' not in st.session_state:
                    ...
                    if st.button("Begin Full-Text Screening", type="primary", use_container_width=True):
                        with st.status("Performing Full-Text Analysis...", expanded=True) as status:
                            # Track start time for full-text screening
                            ft_screening_start_time = time.time()
                            ...
                            def update_paper_progress(completed, total, message):
                                progress_pct = completed / total
                                progress_bar.progress(progress_pct)
                                status_text.text(f"📑 {message}")
                                
                                # Calculate ETA and throughput
                                elapsed = time.time() - ft_screening_start_time
                                if completed > 0 and elapsed > 0:
                                    papers_per_sec = completed / elapsed
                                    remaining = total - completed
                                    eta_seconds = remaining / papers_per_sec if papers_per_sec > 0 else 0
                                    eta_str = format_duration(eta_seconds)
                                    detail_text.caption(f"Speed: {papers_per_sec:.1f} papers/sec | ETA: {eta_str}")
                            
                            # 4. MULTI-AGENT FULL-TEXT SCREENING (PARALLEL BATCH)
                            ...
                            # Convert DataFrame rows to list of dicts for batch processing
                            papers_list = [row.to_dict() for _, row in passed.iterrows()]
                            total_papers = len(papers_list)
                            
                            # Phase indicator
                            status.write(f"🚀 Phase 1/2: Initializing {Config.PARALLEL_AGENT_WORKERS} parallel agents...")
                            time.sleep(0.5)  # Brief pause to show initialization
                            batch_results = ft_orchestrator.screen_papers_batch(
                                papers_list, 
                            ...
                            for idx, (row, res) in enumerate(zip(passed.iterrows(), batch_results)):
                                _, row_data = row
                                is_included = "Include" in str(res.get('decision', ''))
                                entry = row_data.to_dict()
                                entry['Decision'] = "Include" if is_included else "Exclude"
                                entry['Reason'] = res.get('reason', 'N/A')
                                entry['Abstract'] = row_data.get('Abstract', 'N/A')
                            ...
                            status.write(f"✅ Screening complete! Processed {len(batch_results)} papers")

                            st.session_state.full_text_results = pd.DataFrame(final_rows)
                            
                            # Calculate and store full-text screening duration
                            ft_elapsed_time = time.time() - ft_screening_start_time
                            st.session_state.ft_screening_duration = ft_elapsed_time
                            
                            st.session_state.prisma_counts.update({
                                'ft_exclusion_breakdown': ft_reasons, 
                                'included_final': len([d for d in final_rows if "Include" in d['Decision']])
                            })
                            formatted_ft_time = format_duration(ft_elapsed_time)
                            status.update(label=f"✅ Full-Text Analysis Complete! ({formatted_ft_time})", state="complete")
                            st.rerun()
                else:
                    ft_duration = st.session_state.get('ft_screening_duration', 0)
                    st.success(f"✅ Full-text screening complete! ({format_duration(ft_duration)})")
        ...
        if 'full_text_results' not in st.session_state or st.session_state.full_text_results is None:
            ...
            if included_papers.empty:
                ...
                if st.session_state.snowball_results:
                    ...
                    display_cols = ['title', 'source', 'seed_paper_title', 'citation_type', 'url', 'id']
                    ...
                    if st.session_state.snowball_screened is None:
                        ...
                        if st.button("Screen Snowballed Papers", type="primary", use_container_width=True):
                            with st.status("AI-screening snowballed papers...", expanded=True) as status:
                                # Convert snowball papers to list for batch processing
                                snowball_papers_list = st.session_state.snowball_results
                                total_snowball = len(snowball_papers_list)
                                
                                # Create batch progress bar
                                snowball_progress = st.progress(0, text=f"Screening 0/{total_snowball} snowballed papers")
                                
                                def update_snowball_progress(completed, total, message):
                                    progress = completed / total
                                    snowball_progress.progress(progress, text=f"Screening {completed}/{total} papers")
                                
                                # Prepare API keys for thread-safe initialization
                                api_keys = {
                                    'openai': st.session_state.get('openai_api_key', ''),
                                    'anthropic': st.session_state.get('anthropic_api_key', ''),
                                    'gemini': st.session_state.get('gemini_api_key', '')
                                ...
                                # Initialize orchestrator for snowball screening
                                from utils import FullTextOrchestrator
                                snowball_orchestrator = FullTextOrchestrator(
                                    inclusion_criteria=st.session_state.get('inclusion_list', []),
                                    exclusion_criteria=st.session_state.get('exclusion_list', []),
                                    model_name=model_name,
                                    api_keys=api_keys
                                ...
                                # Convert to paper dicts for batch processing
                                papers_for_batch = []
                                for paper in snowball_papers_list:
                                    papers_for_batch.append({
                                        ...
                                        'seed_paper_title': paper.get('seed_paper_title', 'Unknown'),
                                        'discovery_method': paper.get('discovery_method', 'citation_snowball')
                                    })
                                
                                # Batch screen all snowball papers in parallel
                                status.write(f"Screening {total_snowball} papers with parallel agents...")
                                batch_results = snowball_orchestrator.screen_papers_batch(
                                    papers_for_batch,
                                    progress_callback=update_snowball_progress
                                )
                                
                                # Process results
                                screened_rows = []
                                for paper, res in zip(snowball_papers_list, batch_results):
                                ...
                                snowball_progress.empty()
                                status.update(label=f"✅ Screening complete! {len(screened_rows)} papers screened", state="complete")
                            st.rerun()
                    else:
        ...
        if st.session_state.results is not None:
            ...
            if not passed.empty:
                ...
                if 'extracted_papers_data' in st.session_state and st.session_state.extracted_papers_data:
                    ...
                    papers_with_tables = sum(1 for p in st.session_state.extracted_papers_data if p['Extracted_Tables'])
                    total_papers = len(st.session_state.extracted_papers_data)
                    papers_without_tables = total_papers - papers_with_tables
                    
                    # Show warning if many papers lack tables (likely paywalled)
                    if papers_without_tables > 0:
                        with st.expander("⚠️ Table Extraction Limitations", expanded=papers_without_tables > total_papers * 0.5):
                            st.markdown(f"""
                            **{papers_without_tables} of {total_papers} papers** had no extractable tables.
                            
                            This typically means:
                            - **Paywalled/Closed Access**: The full text is not freely available via Europe PMC or PubMed Central
                            - **No Tables**: The paper may not contain tabular data
                            - **PDF Only**: Some papers only provide tables in PDF format which cannot be automatically extracted
                            
                            **Solutions:**
                            1. Look for papers marked "Free Full Text" in PubMed
                            2. Check if papers are available as Open Access (OA)
                            3. Manually download and extract tables from PDFs
                            """)
                    ...
                    for paper_data in st.session_state.extracted_papers_data:
                        with st.expander(f"{paper_data['Paper_Title']}", expanded=False):
                            # Calculate total rows and max columns across all tables
                            total_rows = sum(len(table['data']) for table in paper_data['Extracted_Tables'] if table['data'])
                            max_columns = max((len(table['data'][0]) if table['data'] else 0) for table in paper_data['Extracted_Tables']) if paper_data['Extracted_Tables'] else 0
                            
                            # Paper header with metrics
                            ...
                            with col4:
                                st.metric("Max Columns", max_columns)
                ...
                # Display extracted tables
                            if paper_data['Extracted_Tables']:
                                ...
                                # Debug info
                                if st.checkbox("Show debug info", key=f"debug_{paper_data['Paper_Title'][:20]}"):
                                    st.write(f"Found {len(paper_data['Extracted_Tables'])} tables")
                                    for j, t in enumerate(paper_data['Extracted_Tables']):
                                        st.write(f"Table {j}: {t.get('title', 'No title')} - Rows: {len(t.get('data', []))}")
                                ...
                                for i, (table, tab) in enumerate(zip(paper_data['Extracted_Tables'], table_tabs)):
                                    with tab:
                                        ...
                                        if table_data and len(table_data) > 0:
                                            try:
                                                # Handle single row or inconsistent column counts
                                                ...
                                                # Table actions - Export and View Full Paper on same line
                                                ...
                                                with col_export:
                                                    ...
                                                    if st.button(f"📥 Export", key=f"export_table_{title_hash}_{i}"):
